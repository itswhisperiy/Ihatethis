...
function reconstructComponents(components, sourceMsgId, sourceChId) {
  ...
  return components.map(row => {
    ...
    const buttons = row.components.map(btn => {
      ...
      if (btn.emoji) {
        if (btn.emoji.id) {
          builder.setEmoji({ id: btn.emoji.id, name: btn.emoji.name || 'emoji', animated: btn.emoji.animated || false });
        } else if (btn.emoji.name) {
          builder.setEmoji(btn.emoji.name);
        }
      }
      return builder;
    });
...
// Strip emojis from components for fallback send
function stripEmojis(components) {
  if (!components) return [];
  return components.map(row => {
    const newRow = ActionRowBuilder.from(row);
    const buttons = newRow.components.map(btn => {
      const data = btn.toJSON ? btn.toJSON() : btn.data;
      delete data.emoji;
  ...
  if (parsed.pathname === "/forward" && req.method === "POST") {
    ...
    req.on("end", async () => {
      try {
        ...
        let sent;
        try {
          sent = await destChannel.send(payload);
        } catch (sendErr) {
          // If emoji caused failure, retry without emojis
          if (sendErr.code === 50035 || sendErr.message?.toLowerCase().includes('emoji')) {
            console.log("[Bot] Emoji rejected, retrying without emojis...");
            payload.components = stripEmojis(payload.components);
            sent = await destChannel.send(payload);
          } else {
            throw sendErr;
          }
